...
def user_counter():
    try:
        ...
        command = f"python3 -u -m demucs.separate --out {output_dir} {file_path} -d cpu --mp3"
        ...
        output_path = os.path.join(output_dir, "htdemucs", bucket_name)
        timeout_seconds = 60  # Adjust this based on your requirements
        start_time = time.time()
        while not os.path.exists(output_path):
            print("Waiting for file to be created...")
            time.sleep(timeout_seconds)  # Wait for 60 seconds before checking again
